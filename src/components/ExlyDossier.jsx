...
const coverParagraphs = [
  "Exly Growth is a Meta + Google ads growth partnership for creators, coaches, and infopreneurs who want to scale without percentage-based agency fees.",
  "Our commercial model is simple and clear: Zero Revenue Share. One Fixed Fee. We do not take a cut from your earnings.",
  "Beyond software, Exly runs full-funnel growth execution including ad operations, landing pages, CRM, automation, payments, and reporting.",
];
...
function DefaultCover({ liteMotion = false, compact = false }) {
  return (
    <motion.div
      ...
      animate="show"
      exit="hidden"
      className="mx-auto max-w-3xl p-5 sm:p-6 md:p-12"
    >
    ...
    <MotionConfig reducedMotion="user">
      <LayoutGroup id="exly-dossier">
        ...
        <motion.div
          ...
          style={gridStyle}
          animate={{ scale: activeSection && !shouldLiteMotion ? 0.98 : 1 }}
          transition={{ duration: 0.3 / motionProfile.speed, ease: [0.22, 1, 0.36, 1] }}
        />
        <motion.div className="pointer-events-none absolute inset-0" style={{ backgroundImage: paperLight, opacity: lightOpacity }} />
        <AmbientCartoons liteMotion={shouldLiteMotion} motionProfile={motionProfile} />
        <motion.div
          className="pointer-events-none absolute left-1/2 top-1/2 h-[78vh] w-[72vw] -translate-x-1/2 -translate-y-1/2 rounded-[48px] bg-[radial-gradient(circle,rgba(99,102,241,0.2)_0%,rgba(99,102,241,0)_70%)] blur-2xl"
          initial={{ opacity: 0 }}
          animate={{ opacity: activeSection && !shouldLiteMotion ? 0.75 : 0 }}
          transition={{ duration: 0.34, ease: [0.22, 1, 0.36, 1] }}
        />
        <motion.div
          className="pointer-events-none absolute inset-0 bg-slate-900/30"
          initial={{ opacity: 0 }}
          animate={{ opacity: activeSection ? (isMobile ? 0.34 : 1) : 0 }}
          transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }}
        />

        <div className="relative z-10 flex h-full flex-col">
          <div className="relative flex min-h-0 flex-1">
            {!activeSection && <SectionLearnMoreBrace />}
            <motion.aside
              className="hidden h-full w-[330px] flex-col border-r border-slate-200 bg-white/70 p-5 backdrop-blur-sm md:flex"
              animate={{ scale: activeSection ? 0.995 : 1 }}
              transition={{ duration: 0.24 / motionProfile.speed, ease: [0.22, 1, 0.36, 1] }}
            >
              <div className="mb-5">
                <BrandLogo className="h-10 w-auto object-contain" />
                <h1 className="mt-2 text-xl font-bold leading-snug text-[#111827]">{proposalTitle}</h1>
                <p className="mt-1 text-xs text-[#6B7280]">10 strategic sections</p>
              </div>
              <div className="min-h-0 flex-1">
                <SectionList
                  activeId={activeId}
                  onSelect={openSection}
                  faded={Boolean(activeSection)}
                  playIntro={!sidebarIntroDone}
                  motionProfile={motionProfile}
                />
              </div>
            </motion.aside>

            <main className="relative flex flex-1 flex-col overflow-hidden p-0 md:p-8">
              <div
                className="md:hidden flex shrink-0 items-center border-b border-slate-200 bg-white/90 px-4 backdrop-blur-sm"
              ...
              <div className="relative min-h-0 flex-1">
                <motion.div
                  className={`h-full overflow-y-auto ${activeSection ? "pointer-events-none" : ""}`}
                  style={isMobile ? { paddingBottom: "max(0.75rem, env(safe-area-inset-bottom))" } : undefined}
                  initial={false}
                  animate={{
                    opacity: activeSection ? (isMobile ? 0.52 : 0.42) : 1,
                    y: activeSection && !isMobile ? 2 : 0,
                    filter: activeSection && !isMobile ? "blur(0.7px)" : "blur(0px)",
                  }}
                ...
                <AnimatePresence mode="sync" initial={false}>
                  {activeSection && (
                    <motion.div
                      ...
                      initial={isMobile ? { opacity: 0, y: 10 } : { opacity: 0, scale: 0.994, y: 8 }}
                      animate={isMobile ? { opacity: 1, y: 0 } : { opacity: 1, scale: 1, y: 0 }}
                      exit={
                        isMobile
                          ? { opacity: 0, y: 8, transition: { duration: 0.2, ease: [0.22, 1, 0.36, 1] } }
                          : { opacity: 0, scale: 0.994, y: 6, transition: { duration: 0.26, ease: [0.22, 1, 0.36, 1] } }
                      }
                      transition={{ duration: isMobile ? 0.2 : 0.26, ease: [0.22, 1, 0.36, 1] }}
                    >
                      <ActiveSectionPanel
                        section={activeSection}
                        onClose={closeSection}
                        isMobile={isMobile}
                        liteMotion={shouldLiteMotion}
                        motionProfile={motionProfile}
                      />
                    </motion.div>
                  )}
              ...
              <AnimatePresence>
                {activeSection && !(isMobile && mobileListOpen) && (
                  <StickyDiscoveryFooter
                    onSchedule={openDiscoveryCTA}
                    liteMotion={shouldLiteMotion}
                    isMobile={isMobile}
                  />
                )}
        ...
        <AnimatePresence>
          {isMobile && mobileListOpen && !activeSection && (
            <motion.div
              className="absolute inset-0 z-30 overflow-y-auto overscroll-contain bg-[#F3F4F6] px-4"
            ...
            >
              <div className="mb-4 flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-2 text-lg font-bold leading-snug text-[#111827]">{proposalTitle}</h2>
                </div>
                <button
                  onClick={() => setMobileListOpen(false)}
                  className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-[#111827]"
                >
                  Close
                </button>
              </div>
